...
class IDResolver: ObservableObject {
    ...
    func resolveCanonicalID(for item: ClipboardItem) async -> IDResolutionResult {
        ...
        print("🔍 [iOS IDResolver] Resolving canonical ID for item: \(item.id?.uuidString ?? "unknown")")
        ...
        if let existingCanonicalID = item.canonicalID,
           !item.isTemporary {
            print("✅ [iOS IDResolver] Item already has canonical ID: \(existingCanonicalID.uuidString)")
        ...
        if matchingItems.isEmpty {
            ...
            print("🆕 [iOS IDResolver] Assigned new canonical ID: \(newCanonicalID.uuidString)")
    ...
    func batchResolveCanonicalIDs(for items: [ClipboardItem]) async {
        ...
        print("📦 [iOS IDResolver] Starting batch resolution for \(items.count) items")
        ...
        print("✅ [iOS IDResolver] Batch resolution completed")
    }
    ...
    func resolveAllTemporaryItems() async {
        ...
        do {
            ...
            if !temporaryItems.isEmpty {
                print("🔄 [iOS IDResolver] Found \(temporaryItems.count) temporary items to resolve")
                await batchResolveCanonicalIDs(for: temporaryItems)
            }
        } catch {
            print("❌ [iOS IDResolver] Error fetching temporary items: \(error)")
        }
    }
    ...
    func resolveConflict(_ conflict: IDConflict, using strategy: ResolutionStrategy) async {
        print("⚖️ [iOS IDResolver] Resolving conflict using strategy: \(strategy)")
        ...
        print("✅ [iOS IDResolver] Conflict resolved - using canonical ID: \(winningItem.canonicalID?.uuidString ?? "unknown")")
    }
    ...
    private func findMatchingItems(content: String, timestamp: Date) async -> [ClipboardItem] {
        ...
        do {
            ...
            print("❌ [iOS IDResolver] Error finding matching items: \(error)")
            return []
        }
    }
    ...
    private func handleIDConflicts(localItem: ClipboardItem, matchingItems: [ClipboardItem]) async -> IDResolutionResult {
        ...
        if matchingItems.count == 1 {
            ...
            if let canonicalID = matchingItem.canonicalID {
                ...
                print("🔗 [iOS IDResolver] Resolved using matching item's canonical ID: \(canonicalID.uuidString)")
        ...
        if let bestMatch = bestMatch,
           let canonicalID = bestMatch.canonicalID {
            ...
            print("🎯 [iOS IDResolver] Resolved using best match canonical ID: \(canonicalID.uuidString)")
        ...
        print("⚠️ [iOS IDResolver] Conflict detected - requires resolution")
        return .conflicted(
    ...
    private func updateItemCanonicalID(_ item: ClipboardItem, canonicalID: UUID, initiatingDevice: String) async {
        ...
        await context.perform {
            ...
            do {
                try context.save()
                print("💾 [iOS IDResolver] Updated item canonical ID: \(canonicalID.uuidString)")
            } catch {
                print("❌ [iOS IDResolver] Error updating canonical ID: \(error)")
            }
        }
    }
    ...
    private func handleResolutionResult(_ result: IDResolutionResult, for item: ClipboardItem) async {
        ...
        case .resolved(let canonicalID, let initiatingDevice):
            print("✅ [iOS IDResolver] Successfully resolved: \(canonicalID.uuidString) from \(initiatingDevice)")
            
        case .conflicted(let localID, let remoteID, let reason):
            print("⚠️ [iOS IDResolver] Conflict detected between \(localID.uuidString) and \(remoteID.uuidString): \(reason)")
            
        case .pending(let temporaryID):
            print("⏳ [iOS IDResolver] Resolution pending for: \(temporaryID.uuidString)")
            
        case .failed(let error):
            print("❌ [iOS IDResolver] Resolution failed: \(error)")
        }
    }
