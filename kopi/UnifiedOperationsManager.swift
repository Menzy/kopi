...
class UnifiedOperationsManager: ObservableObject {
    ...
    func editClipboardItem(canonicalID: UUID, newContent: String) async -> UnifiedOperationResult {
    ...
    func deleteClipboardItem(canonicalID: UUID) async -> UnifiedOperationResult {
        // Deleting item (verbose logging disabled)
        ...
        // Delete operation completed (verbose logging disabled)
        return .success(operation: operation)
    }
    
    /// Toggle favorite status across all devices
    func toggleFavorite(canonicalID: UUID) async -> UnifiedOperationResult {
        let isFavorite = !favoriteItems.contains(canonicalID)
        // Setting favorite (verbose logging disabled)
    ...
    func togglePin(canonicalID: UUID) async -> UnifiedOperationResult {
        let isPinned = !pinnedItems.contains(canonicalID)
        // Setting pinned (verbose logging disabled)
    ...
    func addToCollection(canonicalID: UUID, collectionName: String) async -> UnifiedOperationResult {
        // Adding to collection (verbose logging disabled)
    ...
    func batchProcessOperations(_ operations: [UnifiedOperation]) async -> [UnifiedOperationResult] {
        // Batch processing operations (verbose logging disabled)
        ...
        var results: [(Int, UnifiedOperationResult)] = []
        
        // Process operations in parallel where possible
        await withTaskGroup(of: (Int, UnifiedOperationResult).self) { group in
        ...
        // Batch processing completed (verbose logging disabled)
    ...
    func resolveEditConflict(canonicalID: UUID, strategy: ConflictResolutionStrategy) async -> UnifiedOperationResult {
        // Resolving edit conflict (verbose logging disabled)
    ...
    private func processOperation(_ operation: UnifiedOperation) async -> UnifiedOperationResult {
        switch operation {
    ...
    private func executeEditOperation(_ operation: UnifiedOperation, currentItem: ClipboardItem) async -> UnifiedOperationResult {
        ...
        await context.perform {
            ...
            do {
                try context.save()
                // Local edit saved (verbose logging disabled)
            } catch {
                print("❌ [macOS UnifiedOps] Failed to save local edit: \(error)")
            }
        }
    ...
    private func fetchItemByCanonicalID(_ canonicalID: UUID) async -> ClipboardItem? {
        ...
        do {
            ...
            print("❌ [macOS UnifiedOps] Error fetching item: \(error)")
            return nil
        }
    }
    ...
    private func updateItemFavoriteStatus(_ item: ClipboardItem, isFavorite: Bool) async {
        ...
        await context.perform {
            ...
            do {
                try context.save()
                // Updated favorite status (verbose logging disabled)
            } catch {
                print("❌ [macOS UnifiedOps] Failed to update favorite status: \(error)")
            }
        }
    }
    ...
    private func updateItemPinStatus(_ item: ClipboardItem, isPinned: Bool) async {
        ...
        await context.perform {
            ...
            do {
                try context.save()
                // Updated pin status (verbose logging disabled)
            } catch {
                print("❌ [macOS UnifiedOps] Failed to update pin status: \(error)")
            }
        }
    }
    ...
    private func updateItemCollection(_ item: ClipboardItem, collectionName: String, add: Bool) async {
        ...
        await context.perform {
            ...
            do {
                try context.save()
                // Updated collection (verbose logging disabled)
            } catch {
                print("❌ [macOS UnifiedOps] Failed to update collection: \(error)")
            }
        }
    }
    
    private func syncOrganizationChange(_ operation: UnifiedOperation) async {
        // Sync organization changes to CloudKit (silent)
    }
    ...
    func removeFromCollection(canonicalID: UUID, collectionName: String) async -> UnifiedOperationResult {
        // Removing from collection (verbose logging disabled)
    ...
    private func reorderItems(canonicalIDs: [UUID], newOrder: [Int]) async -> UnifiedOperationResult {
        // Reordering items (verbose logging disabled)
            ...
            if let item = await fetchItemByCanonicalID(canonicalID) {
                ...
                await context.perform {
                    ...
                    do {
                        ...
                        print("❌ [macOS UnifiedOps] Failed to update sort order: \(error)")
                    }
                }
            }
        }
