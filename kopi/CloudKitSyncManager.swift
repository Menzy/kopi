...
class CloudKitSyncManager: ObservableObject {
    ...
    /// Force sync all pending changes to CloudKit
    func forceSyncAll() async {
        print("🔄 [CloudKitSync] Starting full sync")
        await MainActor.run { isSyncing = true }
        
        // Fetch pending local changes
        let localChanges = fetchPendingLocalChanges()
        ...
        for change in localChanges {
            await processLocalChange(change)
        }
    ...
    func syncItem(_ item: ClipboardItem, operation: SyncOperation) async {
        print("🔄 [CloudKitSync] Syncing item: \(item.canonicalID?.uuidString ?? "unknown")")
    ...
    func deleteItemAcrossDevices(canonicalID: UUID) async {
        print("🗑️ [CloudKitSync] Deleting item across devices: \(canonicalID.uuidString)")
    ...
    func checkCloudKitAvailability() async -> Bool {
        do {
            ...
            print("❌ [CloudKitSync] CloudKit not available: \(error)")
            return false
        }
    }
    ...
    private func fetchPendingLocalChanges() -> [SyncOperation] {
        ...
        do {
            ...
            print("❌ [CloudKitSync] Error fetching local changes: \(error)")
            return []
        }
    }
    ...
    private func processLocalChange(_ operation: SyncOperation) async {
        ...
        case .resolve(let item):
            ...
            case .resolved(let canonicalID, _):
                print("✅ [CloudKitSync] Resolved item, now syncing: \(canonicalID.uuidString)")
                await syncItemToCloudKit(item)
                
            case .conflicted, .pending, .failed:
                print("⚠️ [CloudKitSync] Resolution failed for item: \(item.id?.uuidString ?? "unknown")")
            }
        }
    }
    ...
    private func syncItemToCloudKit(_ item: ClipboardItem) async {
        guard let canonicalID = item.canonicalID else {
            print("❌ [CloudKitSync] Cannot sync item without canonical ID")
            return
        }
        ...
        do {
            ...
            print("✅ [CloudKitSync] Saved record: \(savedRecord.recordID.recordName)")
        } catch {
            print("❌ [CloudKitSync] Failed to save record: \(error)")
            await MainActor.run {
    ...
    private func deleteItemFromCloudKit(canonicalID: UUID) async {
        ...
        do {
            ...
            print("✅ [CloudKitSync] Deleted record: \(canonicalID.uuidString)")
        } catch {
            print("❌ [CloudKitSync] Failed to delete record: \(error)")
            await MainActor.run {
                syncErrors.append(error)
            }
        }
    ...
    private func fetchRemoteChanges() async {
        ...
        changesOperation.fetchDatabaseChangesResultBlock = { result in
            ...
            case .failure(let error):
                print("❌ [CloudKitSync] Failed to fetch remote changes: \(error)")
                Task {
                    await MainActor.run {
    ...
    private func fetchZoneChanges(zoneID: CKRecordZone.ID) async {
        ...
        operation.recordWasChangedBlock = { recordID, result in
            ...
            case .failure(let error):
                print("❌ [CloudKitSync] Record change error: \(error)")
            }
        }
    ...
    private func handleRemoteRecordChange(_ record: CKRecord) async {
        ...
        do {
            ...
            print("❌ [CloudKitSync] Error processing remote change: \(error)")
        }
    }
    ...
    private func updateLocalItem(_ item: ClipboardItem, with record: CKRecord) async {
        ...
        await context.perform {
            ...
            do {
                try context.save()
                // Updated local item from remote
            } catch {
                print("❌ [CloudKitSync] Failed to update local item: \(error)")
            }
        }
    }
    ...
    private func createLocalItem(from record: CKRecord) async {
        ...
        await context.perform {
            ...
            do {
                try context.save()
                // Created local item from remote
            } catch {
                print("❌ [CloudKitSync] Failed to create local item: \(error)")
            }
        }
    }
    ...
    private func deleteLocalItemsWithCanonicalID(_ canonicalID: UUID) async {
        ...
        await context.perform {
            do {
                ...
                for item in items {
                    context.delete(item)
                    // Deleted local item silently
                }
                try context.save()
            } catch {
                print("❌ [CloudKitSync] Failed to delete local items: \(error)")
            }
        }
    }
    ...
    private func saveChangeToken() {
        ...
        do {
            ...
            print("❌ [CloudKitSync] Failed to save change token: \(error)")
        }
    }
}